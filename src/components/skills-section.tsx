...
const skillCategories = [
  {
    ...
    items: [
      { name: "React", icon: "logos:react", level: "Beginner", years: 0, description: "Building complex web applications with React and its ecosystem" },
      ...
      { name: "TailwindCSS", icon: "logos:tailwindcss-icon", level: "Intermediate", years: 1, description: "Rapid UI development with Tailwind" }
    ]
  },
  // {
  //   name: "Backend",
  ...
  // },
  {
    ...
    items: [
      { name: "Figma", icon: "logos:figma", level: "Beginner", years: 0, description: "UI/UX design and prototyping" },
...
const SkillCard = ({ skill, categoryColor }: { skill: any; categoryColor: string }) => {
  ...
  return (
    ...
    >
      ...
      >
        <div className={`absolute inset-0 bg-gradient-to-br ${categoryColor} rounded-2xl opacity-0 group-hover:opacity-10 transition-opacity duration-300`} />
        <div className={`
          ...
          {skillCategories.map((category) => (
            <div key={category.name} className="space-y-6">
              <div className="flex items-center gap-4">
                <div className={`h-8 w-2 rounded-full bg-gradient-to-b ${category.color}`} />
